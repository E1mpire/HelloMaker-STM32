...
Kinematics::Kinematics(int motor_max_rpm, float wheel_diameter,float base_width, float wheels_x_distance, float wheels_y_distance)
{
...
Kinematics::rpm Kinematics::calculateRPM(float linear_x, float linear_y, float angular_z)
{
    ...
    #if RADIUS_WAY
    tangential_vel = angular_vel_z_mins * base_width_*3/4;
    #else 
...
Kinematics::velocities Kinematics::getVelocities(int rpm1, int rpm2)
{
	...
	#if RADIUS_WAY
    vel.angular_z =  (average_rps_a * wheel_circumference_) / base_width_*4.0/3.0 ;
    #else
